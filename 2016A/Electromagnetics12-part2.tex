...
\begin{equation}
\Phi\equiv\int_{S}\boldsymbol{B}\cdot\boldsymbol{n}\mathrm{d}S
\end{equation}
を磁束と呼び、単位をWb(ウェーバー)で表す。つまり$\mathrm{T=\frac{Wb}{m^{2}}}$。

図電後3-2

(1.25)に(1.12)を代入し、ストークスの定理を用いると、
\begin{align*}
...
\begin{equation}
\therefore(1.25)\Leftrightarrow\Phi=\int_{C}\boldsymbol{A}\cdot\mathrm{d}\boldsymbol{s}
\end{equation}
とも書ける。

\textbf{境界上の$\boldsymbol{A}$を一階積分すると、その中の磁束が求まる。}

$S$が閉曲面のとき、$C$はないので$\Phi=0$⇔(1.24)
...
\begin{equation}
\boldsymbol{m}\equiv IA\boldsymbol{n}
\end{equation}
を定義する。
...
図電後3-8
\end{enumerate}
単位体積あたり$N$この磁気モーメント$\boldsymbol{m}_{i}\left(i=1,2,\cdots,N\right)$があるとき、単位体積あたりの巨視的な磁気モーメントは、
\begin{equation}
\boldsymbol{M}=\sum_{i=1}^{N}\boldsymbol{m}_{1}=N\left\langle \boldsymbol{m}_{i}\right\rangle 
\end{equation}
...
今、磁性体の中を通る経路$C$について(1.21)を考える。

$C$に鎖交する電流$I$として、通常の電流$I_{F}$(自由電流と呼ぶ)肉を得て、磁性体内の微小電流ループの総和(磁化電流)$I_{M}$も考える必要がある。

つまり(1.21)は
\begin{equation}
\oint_{C}\boldsymbol{B}\cdot\mathrm{d}\boldsymbol{s}=\mu_{0}I=\mu_{0}\left(I_{E}+I_{M}\right)
\end{equation}
...
\begin{equation}
\therefore I_{M}=\oint_{C}\boldsymbol{M}\cdot\mathrm{d}\boldsymbol{s}
\end{equation}

ストークスの定理より(2.4)の微分系は
\begin{equation}
...
(2.4)を(2.3)に代入して、
\begin{equation}
\oint_{C}\left(\frac{1}{\mu_{0}}\boldsymbol{B}-\boldsymbol{M}\right)\cdot\mathrm{d}\boldsymbol{s}=I_{F}
\end{equation}

そして、新しい馬
\begin{equation}
\boldsymbol{H}\equiv\frac{1}{\mu}\boldsymbol{B}-\boldsymbol{M}
\end{equation}
を定義すると、(2.6)は、
\begin{equation}
\oint_{C}\boldsymbol{H}\cdot\mathrm{d}\boldsymbol{s}=I_{F}
\end{equation}
→微分系
\begin{equation}
...
・強磁性体でも、ヒステリシスが小さい場合や、最小に磁化する領域で(2.10)を用いる。

・磁石($H=0$で$M\neq0$)は、(2.10)は使えない。$M$=一定 とするか、M-H特性を厳密に考える。
...
\begin{equation}
\therefore\boldsymbol{B}=\mu\boldsymbol{H}
\end{equation}

ただし
\begin{equation}
\mu\equiv\left(1+\chi\right)\mu_{0}
\end{equation}
