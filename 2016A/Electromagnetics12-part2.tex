...
コンデンサに電流を流してでんcを貯める間に、コンデンサの周囲にどのような次回が発生するか?
...
\end{equation}
を上図の閉経路$C$に適用すれば$\boldsymbol{H}$が求まるハず。
...
一方、「閉曲面$S$」のとり方は教会が$C$でありさえすれば任意のはず。(∵ストークスの定理)
...
一般に$\varepsilon,\mu,\sigma$は$\boldsymbol{E}$や$\boldsymbol{H}$の関数になるが、$\boldsymbol{E}$と$\boldsymbol{H}$の反意を限定すれば、多くの物質(線形な物質と呼ぶ。例外は磁石)で、定数と\uline{近似}できる。

\subsection{電磁波}

マクスウェルが導入した変位電流の項($\pd{\boldsymbol{D}}t$)により、\uline{電界と磁界が}

互いに誘導しながら空間を伝わっていく伝承が、初めて説明できる。
